  ...
  <div class="forecast-chart">
    <table class="summarization" cellspacing="0" cellpadding="0" border="0">
      <tr>
        ...
        <td class="text-gray">当前瞬时供水量(m³/h)</td>
      </tr>
      <tr>
        <td width="20%">
          <div class="dot">...</div>
        ...
        <td class="text-gray">今日累计供水量(m³)</td>
      </tr>
      <!--  <tr v-for="item in forecastData.summarization" :key="item.name">
        <td width="20%">
          <div class="dot">...</div>	8507.11	当前瞬时供水量(m³/h)
...
8507.11	今日累计供水量(m³)
        </td>
        <td width="30%">{{sunshiwater}}</td>
        <td width="30%">{{item.value}}sunshiwater</td>
...
export default {
  ...
  mounted() {
    ...
    that.leijiwater = a;
    that.leijiwater2 = a;
    that.flowdsq=setInterval(function() {
      that.sunshiwater = (Math.random() * 1000 + 8200).toFixed(2);
    ...
    that.run = function() {
      ...
      that.index++;
      console.log("%c的值%c", "font-size:24px;color:red", "", that.index);
    };
    that.run();
  },
  methods: {
  ...
  destroyed() {
    ...
    this.run = null;
    that.flowdsq=null;
    window.clearInterval(that.flowdsq)
    console.log(
